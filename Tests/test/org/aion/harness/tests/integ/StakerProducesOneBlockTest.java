...
public class StakerProducesOneBlockTest {
    ...
    public StakerProducesOneBlockTest() throws InvalidKeySpecException {
    ...
    @Test
    @Ignore
    public void testInternalStakerProducesBlocks() throws Exception {
        ...
        System.out.println("Staking blocks should start being produced now...");
        ...
        byte[] seed = rpc.getSeed();
        byte[] oldSeed = rpc.getSeed();
        
        // we want to assert that staking blocks are being produced
        // we will repeatedly call getSeed for a minute, expecting it to change
        int iterations = 60;

        while(Arrays.equals(oldSeed, seed) && iterations > 0) {
            Thread.sleep(1000);
            seed = rpc.getSeed();
            iterations--;
        }
    ...
    @Test
    public void testExternalStakerProducesOneBlock() throws Exception {
        System.out.println("Funding the hard coded address...");
        System.out.println(hardCodedAddress.getAddress());
