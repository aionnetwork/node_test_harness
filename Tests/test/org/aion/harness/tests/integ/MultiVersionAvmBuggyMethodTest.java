 ...
 *
 * A contract is deployed using version 1 and calls into a method that contains a bug that is later
 * fixed in version 2.
 */
...
public class MultiVersionAvmBuggyMethodTest {
    ...
    @Test
    public void testBuggyMethod() throws Exception {
        ...
        // ====== 2. Call into the contract in version 1 of the avm and verify the bug exists. =======
        boolean isBugFixed = false;
        receipt = callContract(avmUtility, contract, isBugFixed);
        ...
        // ====== 4. Call into the contract in version 2 of the avm and verify the bug is fixed. =======
        isBugFixed = true;
        receipt = callContract(avmUtility, contract, isBugFixed);

        // Verify that the transaction succeeded and that we are past the fork point.
        Assert.assertTrue(receipt.transactionWasSuccessful());
        blockNumber = receipt.getBlockNumber();
        Assert.assertTrue(blockNumber.compareTo(FORK_NUMBER) >= 0);
    }
    ...
    private TransactionReceipt callContract(AvmUtility avmUtility, Address contract, boolean isBugFixed) throws InterruptedException, TimeoutException {
        byte[] data = avmUtility.newAvmStreamingEncoder(AvmVersion.VERSION_1).encodeOneString("invokeBuggyMethod").toBytes();
        ...
        String output = (isBugFixed) ? "Concatenation: AB" : "Concatenation: BA";
        return sendTransactionAndScanForOutput(result.getTransaction(), output);
    }
    ...
    private TransactionReceipt sendTransactionAndScanForOutput(RawTransaction transaction, String output) throws InterruptedException, TimeoutException {
        // we want to ensure that the transaction gets sealed into a block and that the output string is seen.
        ...
        IEvent outputIsSeen = new Event(output);
        IEvent transactionProcessedAndOutputSeen = Event.and(transactionProcessed, outputIsSeen);

        FutureResult<LogEventResult> future = listener.listenForEvent(transactionProcessedAndOutputSeen, 5, TimeUnit.MINUTES);
        ...
        // Note we do not have to verify that the output was seen because the event would fail if not seen (we use an AND above).
    ...
    private byte[] produceJarBytes(AvmUtility avmUtility) {
        return avmUtility.produceAvmJarBytes(AvmVersion.VERSION_1, AvmContract.HARNESS_BUG_FIX);
    }
}
